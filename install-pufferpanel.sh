...
read -r -p "Username: " PP_USER
read -r -p "Email: " PP_EMAIL
read -r -s -p "Password: " PP_PASS
echo
read -r -s -p "Confirm Password: " PP_PASS_CONFIRM
echo
...
docker exec -i pufferpanel /pufferpanel/bin/pufferpanel user add \
  --name "$PP_USER" \
  --email "$PP_EMAIL" \
  --password "$PP_PASS" \
  --admin
